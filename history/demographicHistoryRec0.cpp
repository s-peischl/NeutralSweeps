...
int main()
{
    ...
    filename = "demHistN" + str1 + "tau" + str2 + ".dat";
        ...
        for (int i = 0; i < 2*N; i++)
        {
            ...
            if (dem_hist[t][i] == 2*N)
            {
                ...
                if (dem_hist[t-1][i] != 2*N)
                {
                    ...
                    for (int j = 1; j <= t; j++)
                    {
                        //avg_dem[j] += dem_hist[j][1];
                        avg_dem[j] += dem_hist[j][i];
                    }
                }
            }
        }
    ...
    for (int clock = 1; clock <= fixation_time; clock++)
    {
        files << clock << " " << 1.*avg_dem[clock]/Proba_tau << endl;
    }
    files.close();
